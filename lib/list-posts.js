...
/** Decode a base64 (GitHub blob) string as UTF-8 — btoa/atob are Latin1, so we
 *  round-trip through bytes to preserve non-ASCII characters. */
    ...
    files.map(async (f) => {
      ...
      try {
        ...
        if (r.ok) {
          ...
          raw = j && j.content ? decodeBase64Utf8(j.content) : '';
        }
      } catch {
