...
export async function listPosts(env) {
  ...
  const posts = await Promise.all(
    files.map(async (f) => {
      ...
      // seriesOrder arrives as a string from parseFrontmatter's scalar coercion;
      // normalize to a number (or null) for the dashboard.
      const seriesOrderNum = Number(fm.seriesOrder);
      return {
        slug,
        sha: f.sha,
        ...
        seriesTitle: typeof fm.seriesTitle === 'string' ? fm.seriesTitle : '',
        seriesOrder: Number.isFinite(seriesOrderNum) && fm.seriesOrder !== '' && fm.seriesOrder != null ? seriesOrderNum : null,
        // Series plate knobs (for the Studio's Series settings page).
        seriesTotal: Number.isFinite(Number(fm.seriesTotal)) && fm.seriesTotal !== '' && fm.seriesTotal != null ? Number(fm.seriesTotal) : null,
        ...
        // Scheduled publish time (ISO string) if the post is queued; empty otherwise.
        publishAt: typeof fm.publishAt === 'string' && fm.publishAt ? fm.publishAt : '',
      };
    }),
  );
