 ...
 * rendered by the shared pipeline (db/markdown-config sanitize), so it is injected verbatim.
 */
const esc = (s) =>
...
function bylineHtml(row) {
  ...
  return `${avatar}<div class="who"><b>${names}</b><small>${dateHtml}</small></div>`
    + `<span class="nums">${esc(row.reading_min || 1)} min read</span>`
 ...
 */
export function assemblePermalinkHtml(shellHtml, row, canonicalPath, canonicalUrl) {
  return shellHtml
    .split('AYSRZZTITLE').join(esc(row.title))
    .split('AYSRZZDESC').join(esc(row.description || ''))
    .split('AYSRZZID').join(row.slug)
    .split('AYSRZZURL').join(esc(canonicalUrl))
    // canonical + og:url the shell baked from its own /permalink-shell path
