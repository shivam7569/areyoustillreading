...
// The byline strip under the title: date + reading time + tags.
// There is ONE author, so no name and no avatar — on a personal site every essay is by
// the same person and printing it on all of them tells a reader nothing. What is left is
// the part that actually varies per essay.
function bylineHtml(row) {
  ...
  return (dateHtml ? `<div class="who"><small>${dateHtml}</small></div>` : '')
