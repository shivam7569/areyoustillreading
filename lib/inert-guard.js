...
const BLOCKED_ELEMENTS = new Set([
  ...
  'animate', 'animatecolor', 'animatetransform', 'animatemotion', 'set',
]);
...
function isDangerousUrl(value) {
  ...
  if (/^data:(?:text\/(?:html|xml)|application\/(?:xhtml|xml)|image\/svg)/.test(v)) return true;
  return false;
}
...
export function assertInert(html) {
  ...
  let tree;
  // An `eof-in-*` parse error means an unterminated tag/attribute/comment. In this ISOLATED
  // fragment parse5 auto-closes it at end-of-input and the node looks benign — but body_html is
  // later spliced INTO the shell document, where a dangling `<img src="…` (etc.) swallows the
  // shell TAIL, including the per-request CSP nonce, up to the next quote and exfiltrates it via a
  // permitted https image. Legitimate browser-serialized output is always well-formed (no such
  // errors), so any eof-in-* fails CLOSED — it closes the fragment-vs-document parse differential.
  let dangling = false;
  try {
    tree = parseFragment(raw, { onParseError: (err) => { if (err.code && err.code.startsWith('eof-')) dangling = true; } });
  } catch {
    throw new InertError('content could not be parsed for vetting');
  }
  if (dangling) throw new InertError('unterminated markup is not allowed');
