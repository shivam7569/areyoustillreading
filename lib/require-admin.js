...
export async function requireAdmin(request, env) {
  ...
  if (!userRes.ok) {
    // A 5xx from Supabase is an upstream hiccup, not an invalid session — surface it
    // as 502 so a real admin isn't wrongly told to sign in again during a blip.
    if (userRes.status >= 500) return { ok: false, status: 502, error: 'Could not verify session' };
  ...
  try {
    ...
    // A 200 whose body isn't JSON (misconfigured SUPABASE_URL, a proxy page, etc.) must
    // NOT throw out of the gate — treat it as an upstream failure, not a crash.
    return { ok: false, status: 502, error: 'Could not read session response' };
  }
  ...
  // A non-OK admins lookup (rate limit / transient 5xx) must NOT be read as "not an
  // admin" — that would 403-lock a real owner during a blip. Treat it as upstream 502.
  if (!adminRes.ok) return { ok: false, status: 502, error: 'Could not verify authorization' };
  let admins;
  try {
    admins = await adminRes.json();
  } catch {
    return { ok: false, status: 502, error: 'Could not read authorization response' };
  }
