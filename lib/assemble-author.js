...
function fieldChip(f, i, handle, extra) {
  ...
  return `<a class="fchip${extra ? ' extra' : ''}" href="/@${esc(handle)}/f/${esc(f.slug)}">`
    + `<span class="plate" data-field-plate="${esc(spec)}" style="--wx:${w[0]}%;--wy:${w[1]}%;--wo:${w[2]}%"></span>`
    + `<div class="fb"><h3>${esc(f.title)}</h3>`
    + `<span class="sl">${plural(f.series_count, 'series', 'series')} · ${f.published_parts} of ${f.total_parts} parts published</span>`
...
function workRow(p, retention) {
  ...
  return `<article class="post"><div class="pmain"><span class="picon" aria-hidden="true"></span><div class="ptext">`
    ...
    + `</div></div><div class="aside"><span class="date">${esc(fmtDate(new Date(p.pub_date)))}</span><span class="rt">${esc(p.reading_min || 1)} min</span>${curve}</div></article>`;
}
